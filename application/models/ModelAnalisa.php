...
class ModelAnalisa extends CI_Model
{
    ...
    function hitungMatrixPerbandinganKeriteria($keriteria)
    {
        ...
        $Matrix5 = array(
            "N" => count($keriteria), "Xmaks" => $rata_rata, "IR" => 1, 12,
    ...
    public function getListPeringkat()
    {
        $this->db->select('id_peringkat, tanggal, p.user, username');
        $this->db->from('peringkat_alternatif p');
        $this->db->join('user u', 'p.user = u.id_user');
        return $this->db->get();
    }

    public function getPeringkatById($Where)
    {
    ...
    public function getLatestPeringkat()
    {
        $this->db->select('id_detail, peringkat, t.nama, t.latitude, t.longitude, jumlah_nilai');
        $this->db->from('peringkat_alternatif_detail p');
        $this->db->join('tempat_latihan t', 'p.id_tempat_latihan = t.id_tempat_latihan');
